...
页面结构（侧栏导航）：
- 个股分析：侧栏选股票与日期，主区为「行情分析 / 策略回测」两个 Tab
...
def _render_kline_tab(ts_code, start, end, stocks):
    """行情分析 Tab 内容：K 线 + 指标 + 趋势线/浪型/背离结构分析。"""
...
def _render_backtest_tab(ts_code, start, end):
    """策略回测 Tab 内容。"""
    st.subheader("策略回测")
...
def _render_stock_tabs(ts_code, start, end, stocks):
    tab1, tab3 = st.tabs(["行情分析", "策略回测"])
    with tab1:
        _render_kline_tab(ts_code, start, end, stocks)
    with tab3:
...
def _render_favorites_page(start, end, stocks, code_to_label):
    ...
    try:
        ...
        return
    if fav_df.empty:
        st.info("暂无收藏。请到「个股分析」页，点股票旁的 ☆ 添加收藏。")
        return

    fav_codes = fav_df["ts_code"].tolist()
    current = st.session_state.ts_code
    if current not in fav_codes:
        current = fav_codes[0]
        st.session_state.ts_code = current

    col_list, col_main = st.columns([1.4, 5.5], gap="medium")
    with col_list:
        st.markdown("**我的收藏**")
...
with st.sidebar:
    st.header("导航")
    nav = st.session_state.get("nav", "个股分析")
    if nav not in _PAGES:  # 兼容旧会话里的 "首页" 等值
        nav = "个股分析"
    ...
    for _page in _PAGES:
        if st.button(
            ...
            key=f"nav_{_page}",
            use_container_width=True,
            type="primary" if _page == nav else "secondary",
        ):
            if _page != nav:
...
if nav == "个股分析":
    _render_stock_tabs(ts_code, start, end, stocks)
elif nav == "收藏":
    _render_favorites_page(start, end, stocks, code_to_label)
elif nav == "资金集中度":
