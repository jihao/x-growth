...
# -*- coding: utf-8 -*-
"""
补齐 MySQL astocks_qfq 中缺失的 A 股前复权日线数据到昨天。

功能对齐 backfill_qfq_to_yesterday.py：
...
def parse_args():
    parser = argparse.ArgumentParser(description="补齐 MySQL A 股前复权日线数据到昨天")
    parser.add_argument(
        "--end-date",
        default=default_end_date(),
        help="补齐截止日期，默认昨天，格式 YYYYMMDD 或 YYYY-MM-DD",
    )
    parser.add_argument(
        "--start-date",
        default=None,
        help="强制指定扫描起始日期，格式 YYYYMMDD 或 YYYY-MM-DD",
    )
...
def main():
    ...
    cursor.execute("SELECT MAX(trade_date) FROM daily_qfq")
    row = cursor.fetchone()
