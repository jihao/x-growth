...
仅迁移行情表 `stocks` / `daily_qfq`，不迁移 `app_*`。现有 SQLite 脚本（`update_daily.py`、`backfill_qfq_to_yesterday.py`、`bs_qfq_downloader.py`）保持不变；MySQL 使用新增脚本。
...
在 `database/mysql.env` 中填写（该文件已被 gitignore，勿提交密码）：
...
也可直接导出同名环境变量；已存在的环境变量优先于 `mysql.env`。

### 建表与全量迁移

迁移前请停止对 SQLite 的写入（例如正在运行的 backfill）。

```bash
cd database
python migrate_sqlite_to_mysql.py
# 若目标表已有数据需重导：
python migrate_sqlite_to_mysql.py --force
```

脚本会执行 `schema_mysql.sql`，导入 `stocks` 与 `daily_qfq`，并校验行数、股票数、日期范围与收盘价合计。
...
python backfill_qfq_to_yesterday_mysql.py --end-date 2026-07-22
```
